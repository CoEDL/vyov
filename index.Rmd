---
runtime: shiny
title: "VYOV: Visualise your own vowels <br><h1>A short introduction to Praat for complete beginners</h1>"
author: "Nay San <p> <nay.san@anu.edu.au> </p>"
output:
    tufte::tufte_html:
        toc: true
...
<!-- Retrieve and format date of last commit to repository //-->
Last updated `r git2r::init("/srv/shiny-server") %>% git2r::commits() %>% .[[1]] %>% attr('author') %>% attr('when') %>% as("POSIXct") %>% format('%d %B, %Y')`
</p>
...
## What is a phonetics analysis anyway?

Praat assists us in carrying out phonetic analyses of speech sounds, but just what are phonetic analyses anyway? *Very* roughly stated, they are comparisons of acoustic measurements between two or more groups of speech sounds, and Praat helps us deal with the *practicalities* of grouping speech sounds of interest (e.g. vowels vs. consonants, interrogative vs. declarative sentences), and extracting acoustic measurements of interest (e.g. pitch, intensity, formant structure) for comparison.
...
The goal of the workshop will be to produce a short report with 5 sections (described below). Don't worry if you don't understand all of the vocabulary in the report text—you'll be familiar with them by the end of the workshop!

> In this report, we compare first and second formant averages of five vowels from a participant NS with formant averages reported for Australian-English speakers in @cox2006acoustic.`r side_note("Describe the aim of your analysis.")`
> We recorded five words in the /hVd/ context (*heed, hard, horde, who'd, hair'd*) using the built-in microphone on a 2016 Macbook Pro and in a relatively quiet meeting room at the Australian National University.`r side_note("Describe items recorded and recording environment.")`
...
> The annotation and formant data were extracted from Praat as CSV files and imported into a Google Sheets document as individual sheets named `annotations` and `formants`, respectively. For each row in the `annotations` sheet, average first and second formants were calculated using the `QUERY()` function.`r side_note("Describe data extraction and analysis method.")`

<figure style="width:1000px">
<img src="media/google-sheets.png">
</figure>

> `r margin_note("<b>Note</b>. *Always* give a worked example when describing your data analysis method!")`For example, as seen in figure above, the `F1_average` of 313.503 Hz for the vowel *heed* was calculated by querying the formants sheet for the average of values in column C, but only where the corresponding values in column A were greater than or equal to 0.427148 and less than or equal to 0.74984 (i.e. between `tmin` and `tmax` of *heed*): &#39;`=QUERY(formants!A:C, "select avg(C) where A >= 0.427148 and A <= 0.74984", 1)`&#39;.
...
> The calculated F1 and F2 averages were located on a scatterplot along with average values for AusE male and female speakers from @cox2006acoustic. As observed in the figure above, the acoustic data indicated that NS's <em>heed</em> vowel was produced in a location effectively identical to the AusE Male average, while their <em>who'd</em> was relatively more backed than both the AusE female and male averages.`r side_note("Produce vowel plot, and make some observations.")`
...
One way that we can we can incorporate altitude information into a map is to use a color gradient to indicate the metres above sea level. For example, we can see a small patch of white in the circle below, indicating the group of peaks above 3000 m.

<figure style="width:800px">
<img src="media/alps-topo.png">
</figure>

Similarly, we can indicate how much sound energy there is at a point in time using a grayscale gradient (darker = more energy, lighter = less energy). Just as a terrain map indicates how altitude varies by latitude and longitude, a spectrogram indicates how sound energy varies by frequency (e.g. 0 - 5000 Hz) and time (e.g. 0.2323 - 0.8181 s):

<figure style="width:700px">
<img src="media/spec-sep.png">
</figure>

Notice above how Slice B has a bigger bump at the start, compared to Slice A? In other words, there's more acoustic energy at the lower frequencies in Slice B than in Slice A. We see this in the spectrogram as the dark band at the bottom of Slice B, and the relative lightness at the bottom of Slice A.
...
3. Click `Record` to begin recording your vowels and `Stop` to finish; give a second or so before and after your word list in order to make sure you capture all the words.
    a. Press `Play` to playback the most recent recording, or `Record` again to overwrite it.
...
## Extracting data into Google Sheets
